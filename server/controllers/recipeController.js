...
exports.homepage = async(req,res)=>{
  try{
    const limitNumber = 6;
    const categories = await Category.find({}).limit(limitNumber);// finds all the elements of the database and filters only first 6
    const latest = await Recipe.find({}).sort({_id:-1}).limit(limitNumber);// here this returns an array,this will be made into an object for easy usage and display of the data it possess 
    ...
    const latest_items = {thai,american,chinese,spanish,mexican,indian,latest};
    res.render('index', {title:'Cooking Blog - Home', categories,latest_items});// by default the system goes to views and searches for a template called index, then it passes title's content as an object 
  }
...
exports.exploreCategoriesById = async(req, res) => { 
  try {console.log('Category ID:', req.params.id);
...
exports.searchRecipe = async (req, res) => {
  try{
    ...
    // $text tells mongodb that we are using text as search data type 
    // $search splits the searchTerm by words and runs search for each words and displays the results 
    // diacriticsensitive works on differentiating accents in english letters 
...
exports.exploreRandom = async (req, res) => {
    try {
      let count = await Recipe.find().countDocuments();// counts the total number of docs in the cluster 
      let random = Math.floor(Math.random() * count);
      //let recipe = await Recipe.findOne().skip(random).exec() here exec is used to render the function async'ly ;
      let recipe = await Recipe.findOne().skip(random);// for findOne, it usually starts counting from top, here it starts counting from random, thereby skiping 'random' numbers of docs
...
exports.submitRecipe = async (req, res) => {
  const infoErrorsObj = req.flash('infoErrors');// flashes info errors
  const infoSubmitObj = req.flash('infoSubmit');// flashes info submit 

  res.render('submit-recipe',{title:'Cooking Blog - Submit Recipe', infoErrorsObj , infoSubmitObj});// passed into submit-recipe page 

};

exports.submitRecipeOnPost = async(req, res) => {
  try {
// here the requests come directly from the browser and req.files is where all the files uploaded are put in by express-filemanager package
    let imageUploadFile;
    let uploadPath;
    let newImageName;
    ...
    if(!req.files || Object.keys(req.files).length === 0){
      ...
      imageUploadFile = req.files.image;// image url from req.files
      newImageName = Date.now() + imageUploadFile.name;// making a new image name

      uploadPath = require('path').resolve('./') + '/public/uploads/' + newImageName;
      // path is a default module 
      // resolve('./') is used to make the absolute path into the one of the current working directory which is Cooking - blog project folder
      // /public/uploads/ moves the path to the uploads folder and lastly the image name completes the path

      imageUploadFile.mv(uploadPath, function(err){
        if(err) return res.satus(500).send(err);
      });// this function uploads it into the directory ( moves it into the directory )
    ...
    const newRecipe = new Recipe({
      ...
      image: newImageName
    });// adds a new recipe in to the DB
    
    await newRecipe.save();// saves it into the database 
...
//  FUNCTIONS FOR DELETE AND UPDATE THE RECIPES 
...
},
{
  ...
  "image": "chinese-steak-tofu-stew.jpg",
    ...
    console.log("Dummy data inserted");
            ...
            console.log("Dummy data already exists, no insertion needed");
        }
    } catch (error) {
        console.log(error);
    }
...
// dont confuse routes and controllers,as routes just route but all crucial functions are done in the controllers only 
